...
if(trim($result) == 'yes') {
    ...
    createTable('user',                     'CREATE TABLE IF NOT EXISTS user (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, firstname VARCHAR(150) NOT NULL, lastname VARCHAR(150) NOT NULL, password_hash VARCHAR(100) NOT NULL, email VARCHAR(100) NOT NULL, email_verified TINYINT(1) DEFAULT 0, email_verification_hash VARCHAR(20), active TINYINT(1) DEFAULT 1, created_at DATETIME NOT NULL)', $mysqli);
    ...
    createTable('category',                 'CREATE TABLE IF NOT EXISTS category (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, created_by INT NOT NULL, name VARCHAR(100) NOT NULL, description TEXT NOT NULL, active TINYINT(1) DEFAULT 1, parent INT, CONSTRAINT fk_category_user_id FOREIGN KEY (created_by) REFERENCES user(id), CONSTRAINT fk_category_parent FOREIGN KEY (parent) REFERENCES category(id))', $mysqli);
    createTable('product',                  'CREATE TABLE IF NOT EXISTS product (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, created_by INT NOT NULL, name VARCHAR(100) NOT NULL, description TEXT NOT NULL, image VARCHAR(100) NOT NULL, cost_per_item DECIMAL(7,2) NOT NULL, active TINYINT(1) DEFAULT 1, search_tags TEXT NOT NULL, CONSTRAINT fk_product_created_by FOREIGN KEY (created_by) REFERENCES user(id))', $mysqli);
    createTable('administration_access',    'CREATE TABLE IF NOT EXISTS administration_access (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, user_id INT NOT NULL, created_by INT, created_at DATETIME NOT NULL, CONSTRAINT fk_administration_access_user_id FOREIGN KEY (user_id) REFERENCES user(id), CONSTRAINT fk_administration_access_user_created_by FOREIGN KEY (created_by) REFERENCES user(id))', $mysqli);
    createTable('order_object',             'CREATE TABLE IF NOT EXISTS order_object (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, user_id INT NOT NULL, payment_reference_id INT NOT NULL, shipping_address_id INT NOT NULL, billing_address_id INT, order_status VARCHAR(100) DEFAULT "pending_payment", shipment_data TEXT, created_at DATETIME NOT NULL, CONSTRAINT fk_order_user_id FOREIGN KEY (user_id) REFERENCES user(id), CONSTRAINT fk_order_payment_reference_id FOREIGN KEY (payment_reference_id) REFERENCES payment_reference(id), CONSTRAINT fk_order_shipping_address_id FOREIGN KEY (shipping_address_id) REFERENCES address(id), CONSTRAINT fk_order_billing_address_id FOREIGN KEY (billing_address_id) REFERENCES address(id))', $mysqli);
...
if(trim($result) == 'yes') {
    ...
    $queryResult = $mysqli->query(
        'INSERT INTO user (firstname, lastname, password_hash, email, email_verified, created_at)
        VALUES (\'' . $firstName . '\', \'' . $lastName . '\', \'' . $passwordHash . '\', \'' . $email . '\', 1, \'' . date('Y-m-d H:i:s') . '\')
    ');
